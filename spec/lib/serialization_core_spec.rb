...
describe FastJsonapi::ObjectSerializer do
  ...
  context "when testing class methods of serialization core" do
    it "returns correct hash when id_hash is called" do
      ...
      inputs.each do |hash|
        result_hash = MovieSerializer.id_hash(hash[:id])
        expect(result_hash[:id]).to eq hash[:id]
      end

      result_hash = MovieSerializer.id_hash(nil)
      expect(result_hash).to be nil
    end

    it "returns correct hash when attribute`s_hash is called" do
      attributes_hash = MovieSerializer.attributes_hash(movie)
      ...
      MovieSerializer.attributes_to_serialize.each do |key, attribute|
        ...
        expect(value).to eq movie.public_send(attribute.method)
      end
    end
    ...
    it "returns the correct empty result when relationships_hash is called" do
      ...
      expect(relationships_hash[:actors]).to eq([])
      expect(relationships_hash[:owner]).to eq(nil)
    end
    ...
    it "returns correct values when relationships_hash is called" do
      ...
      expect(relationships_hash[:actors]).to match_array actors_hash
      expect(relationships_hash[:owner]).to eq owner_hash
    end

    it "returns correct hash when record_hash is called" do
      record_hash = MovieSerializer.record_hash(movie, nil)
    ...
    it "serializes known included records only once" do
      ...
      [movie, movie].each do |record|
        included_records.merge! MovieSerializer.get_included_records(
          record,
          includes_list,
          known_included_objects,
          {},
      ...
      expect(included_records[:actors].size).to eq 3
    end
  end
end
